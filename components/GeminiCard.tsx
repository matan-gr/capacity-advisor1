  ...
  const renderContent = (text: string) => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('###')) {
            ...
            elements.push(
                <div key={`h3-${i}`} className="flex items-center gap-2 mt-6 mb-4 pb-1.5 border-b border-slate-100 dark:border-slate-800">
                    <span className="text-indigo-500 dark:text-indigo-400"><Icons.Info size={14} /></span>
                    <h3 className="text-xs font-black uppercase text-slate-800 dark:text-slate-100 tracking-widest leading-none">
                        {content}
                    </h3>
                </div>
            );
        ...
        if (line.startsWith('>')) {
            ...
            elements.push(
                <div key={`bq-${i}`} className={`${bgClass} px-4 py-3 rounded-r-lg mb-4 text-sm leading-relaxed ${textClass}`}>
        ...
        if (line.startsWith('* ') || line.startsWith('- ')) {
            ...
            elements.push(
                <div key={`li-${i}`} className="flex items-start gap-3 mb-2.5 pl-2">
                   <div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-slate-600 shrink-0"></div>
        ...
        if (line.startsWith('[ ]') || line.startsWith('[x]')) {
             ...
             elements.push(
                 <div key={`chk-${i}`} className="flex items-start gap-3 mb-2.5 p-2.5 rounded-lg bg-slate-50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800 transition-all hover:border-slate-200 dark:hover:border-slate-700">
