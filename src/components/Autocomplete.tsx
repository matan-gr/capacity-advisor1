...
const Autocomplete: React.FC<AutocompleteProps> = React.memo(({ options, value, onChange, label, placeholder }) => {
  ...
  return (
    <div className="relative group" ref={wrapperRef}>
      <label className="block text-[10px] font-bold uppercase text-slate-500 mb-2 tracking-wider group-focus-within:text-indigo-500 transition-colors">
        {label}
      </label>
      ...
      {isOpen && (
        <div 
            ref={listRef}
            className="absolute z-50 w-full mt-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl max-h-[500px] overflow-y-auto custom-scrollbar animate-enter transform origin-top ring-1 ring-black/5"
        >
          {Object.keys(groupedOptions).length === 0 ? (
            ...
            Object.entries(groupedOptions).map(([family, items]: [string, MachineTypeOption[]]) => (
                <div key={family}>
                    <div className="sticky top-0 z-10 bg-slate-100/95 dark:bg-slate-800/95 px-4 py-2 border-b border-slate-200 dark:border-slate-700 backdrop-blur-sm shadow-sm">
                        <span className="text-[9px] font-black uppercase text-slate-500 tracking-widest">{family}</span>
                    </div>
                    {items.map((option) => {
