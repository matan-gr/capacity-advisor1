...
Language: ${navigator.language}
  ...
  return (
    <div 
      className="fixed bottom-0 left-0 right-0 bg-white dark:bg-[#0f172a] border-t border-slate-200 dark:border-slate-700 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] dark:shadow-[0_-10px_40px_rgba(0,0,0,0.5)] z-[9999] flex flex-col transition-all duration-300 ease-out"
      style={{ height: `${height}px` }}
    >
      {/* Resizer Handle */}
      <div 
        className="w-full h-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-indigo-500 cursor-ns-resize transition-colors flex justify-center items-center group absolute top-0 left-0 z-20"
        onMouseDown={(e) => {
            e.preventDefault();
      ...
      {/* Header */}
      <div className="bg-slate-50 dark:bg-[#020617] px-4 py-2 flex flex-col md:flex-row items-start md:items-center justify-between border-b border-slate-200 dark:border-slate-800 select-none pt-3 gap-3 md:gap-0">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto overflow-x-auto custom-scrollbar pb-1 md:pb-0">
            <div className="flex bg-white dark:bg-slate-800/50 rounded-lg p-0.5 border border-slate-200 dark:border-slate-700/50 whitespace-nowrap">
                ...
                <button
                    ...
                    className={`px-3 py-1 text-[9px] font-bold uppercase tracking-wider transition-all rounded-md ${
                        activeTab === tab ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'
                    }`}
                >
                    {tab}
                </button>
            ...
            <button onClick={onToggle} className="hidden md:block text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors bg-white dark:bg-slate-800/50 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700/50">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7 7" /></svg>
            </button>
        </div>
      </div>
      ...
      {/* Content */}
      <div className="flex-grow overflow-auto p-6 custom-scrollbar bg-white dark:bg-[#0f172a]">
