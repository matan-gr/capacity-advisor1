...
Language: ${navigator.language}
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-700/50">
              <h4 className="text-[10px] font-bold text-slate-400 uppercase mb-2 flex justify-between">
                  <span>Operation Summary</span>
                  <span className="text-indigo-400">{data.mode.toUpperCase()} MODE</span>
              </h4>
              <pre className="text-xs font-medium text-slate-300 whitespace-pre-wrap font-mono-dense break-words leading-relaxed">
              ...
              <div className="mt-4 pt-4 border-t border-slate-800 grid grid-cols-2 gap-4">
                 <div>
                    ...
                    <span className="text-lg font-mono-dense text-white">{totalLatency}ms</span>
                 </div>
                 <div>
                    <span className="text-[9px] text-slate-500 uppercase block">Timestamp</span>
                    <span className="text-xs font-mono-dense text-slate-300">{data.startTime ? new Date(data.startTime).toLocaleTimeString() : '--:--'}</span>
                 </div>
              </div>
            </div>
            <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-700/50">
      ...
      case 'logs':
        return (
          <div className="relative h-full flex flex-col">
            <div className="flex justify-between items-center mb-2 sticky top-0 z-10 bg-[#0f172a] py-1 border-b border-slate-800 gap-4">
               <div className="relative flex-grow">
                   <input 
                     ...
                     className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-[10px] text-white focus:border-indigo-500 outline-none focus:ring-1 focus:ring-indigo-500"
                   />
               </div>
               <button onClick={copyAllLogs} className="text-[10px] bg-slate-700 hover:bg-slate-600 text-white px-3 py-1 rounded border border-slate-600 transition-colors whitespace-nowrap">Copy All</button>
            </div>
            <div className="space-y-1 overflow-auto custom-scrollbar flex-grow pb-4">
              {filteredLogs.length === 0 && <div className="text-slate-600 text-[10px] italic p-2">No logs match your filter.</div>}
              {filteredLogs.map((log, i) => {
                const isExpanded = expandedLogs.includes(i);
                const isLong = log.message.length > 200;
                return (
                  <div key={i} className="flex gap-3 text-[11px] font-mono-dense border-b border-slate-800/50 pb-2 mb-2 last:border-0 items-start hover:bg-white/5 p-1 rounded transition-colors group">
                    <span className="text-slate-600 shrink-0 select-none w-20 group-hover:text-slate-400 transition-colors">{log.timestamp.split('T')[1].replace('Z','')}</span>
                    <span className={`uppercase font-bold shrink-0 w-12 select-none text-center rounded px-1 text-[9px] py-0.5 ${
                        log.level === 'error' ? 'bg-red-900/30 text-red-400 border border-red-900/50' : 
                        log.level === 'warn' ? 'bg-amber-900/30 text-amber-400 border border-amber-900/50' : 
                        'bg-emerald-900/30 text-emerald-400 border border-emerald-900/50'
                    }`}>{log.level.substring(0,4)}</span>
                    <div className="flex-grow min-w-0">
                      <div 
                        className={`text-slate-300 break-words whitespace-pre-wrap ${!isExpanded && isLong ? 'cursor-pointer' : ''}`}
                      ...
                      {isLong && (
                        <button 
                          ...
                          className="text-[9px] text-indigo-400 hover:text-indigo-300 mt-1 underline decoration-dashed"
                        >
          ...
          <div className="flex flex-col h-full gap-4">
             <div className="p-3 bg-slate-900 border border-slate-700 rounded-lg">
                <div className="flex justify-between items-center mb-2">
                   ...
                   <span className="text-[9px] bg-indigo-900/40 text-indigo-300 px-2 py-0.5 rounded font-bold">{data.request.method}</span>
                </div>
                <div className="text-xs font-mono text-white break-all">{data.request.url}</div>
             </div>
             
             {data.request.curl && (
               <div className="p-3 bg-slate-900 border border-slate-700 rounded-lg relative group">
                  <div className="flex justify-between items-center mb-2">
                     <span className="text-[10px] font-bold text-slate-500 uppercase">cURL Command</span>
                     <button onClick={() => copyToClipboard(data.request?.curl || '')} className="text-[9px] text-indigo-400 hover:text-white transition-colors">Copy</button>
                  </div>
                  <pre className="text-[10px] font-mono text-slate-300 whitespace-pre-wrap break-all max-h-32 overflow-y-auto custom-scrollbar">{data.request.curl}</pre>
               </div>
             )}

             <div className="flex-grow flex flex-col min-h-0 bg-slate-900 border border-slate-700 rounded-lg p-3 overflow-auto custom-scrollbar">
            ...
            <div className="flex flex-col h-full gap-4">
               <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                  <div className="flex items-center gap-2">
                     <Icons.Cloud />
                     <span className="text-xs font-bold text-slate-200">Gemini 3 Flash Preview</span>
                  </div>
               ...
               <div className="flex-1 min-h-0 flex flex-col">
                  <div className="flex justify-between items-center mb-2">
                      <h5 className="text-[10px] font-bold text-slate-400 uppercase">System Prompt</h5>
                      <button onClick={() => copyToClipboard(data.geminiDebug?.prompt || '')} className="text-[9px] text-indigo-400 hover:text-white">Copy</button>
                  </div>
                  <div className="bg-slate-900 border border-slate-700 rounded p-3 overflow-y-auto max-h-[150px] custom-scrollbar">
                     <pre className="text-[10px] text-emerald-300 font-mono whitespace-pre-wrap">{data.geminiDebug.prompt}</pre>
                  </div>
               </div>

               <div className="flex-1 min-h-0 flex flex-col">
                  <div className="flex justify-between items-center mb-2">
                      <h5 className="text-[10px] font-bold text-slate-400 uppercase">Raw Model Response</h5>
                      <button onClick={() => copyToClipboard(data.geminiDebug?.responseRaw || '')} className="text-[9px] text-indigo-400 hover:text-white">Copy</button>
                  </div>
                  <div className="bg-slate-900 border border-slate-700 rounded p-3 overflow-y-auto flex-grow custom-scrollbar">
                     <pre className="text-[10px] text-blue-300 font-mono whitespace-pre-wrap">{data.geminiDebug.responseRaw}</pre>
                  </div>
               </div>
            </div>
         );
        ...
        return (
          <div className="space-y-3 pb-4">
            {data.network.map(entry => (
              <div key={entry.id} className="bg-slate-900/50 p-3 rounded border border-slate-700/50 hover:border-slate-600 transition-colors group">
                <div className="flex justify-between items-start mb-2">
                   <div className="flex items-center gap-2">
                     <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${entry.method === 'POST' ? 'bg-indigo-900/40 text-indigo-300' : 'bg-slate-700 text-slate-300'}`}>{entry.method}</span>
                     <span className="text-[10px] text-slate-500">{entry.timestamp.split('T')[1].replace('Z','')}</span>
                     {entry.latencyMs !== undefined && (
                        <span className={`text-[9px] px-1.5 py-0.5 rounded font-mono ${entry.latencyMs > 500 ? 'text-amber-400 bg-amber-900/20' : 'text-emerald-400 bg-emerald-900/20'}`}>
                            {entry.latencyMs}ms
                        </span>
                     )}
                     {entry.status && (
                        <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${entry.status >= 400 ? 'text-red-400' : 'text-green-400'}`}>
                            HTTP {entry.status}
                        </span>
                     )}
                   </div>
                ...
                <div className="text-[10px] text-slate-300 font-mono-dense break-all bg-black/30 p-2 rounded mb-2 border border-black/50 select-text">
                  {entry.url}
                </div>
                <details className="group/details">
                    <summary className="text-[9px] font-bold text-slate-500 cursor-pointer hover:text-indigo-400 select-none flex items-center gap-1">
                    ...
                    <div className="mt-2 pl-2 border-l border-slate-700 font-mono text-[10px]">
  ...
  return (
    <div 
      className="fixed bottom-0 left-0 right-0 bg-[#0f172a] border-t border-slate-700 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] z-[9999] flex flex-col transition-all duration-300 ease-out"
      style={{ height: `${height}px` }}
    >
      {/* Resizer Handle */}
      <div 
        className="w-full h-1.5 bg-slate-800 hover:bg-indigo-500 cursor-ns-resize transition-colors flex justify-center items-center group absolute top-0 left-0 z-20"
        onMouseDown={(e) => {
            e.preventDefault();
      ...
      >
        <div className="w-16 h-1 bg-slate-600 rounded-full opacity-50 group-hover:opacity-100 transition-opacity"></div>
      </div>
      ...
      <div className="bg-[#020617] px-4 py-2 flex items-center justify-between border-b border-slate-800 select-none pt-3">
        <div className="flex items-center gap-4">
          ...
          <span className={`text-[9px] px-2 py-0.5 rounded font-bold uppercase border ${
            data.status === 'error' ? 'bg-red-900/50 text-red-400 border-red-900' : 
            data.status === 'running' ? 'bg-amber-900/50 text-amber-400 animate-pulse border-amber-900' : 
            ...
            <div className="flex bg-slate-800/50 rounded-lg p-0.5 border border-slate-700/50">
                ...
                <button
                    ...
                    className={`px-3 py-1 text-[9px] font-bold uppercase tracking-wider transition-all rounded-md ${
                        activeTab === tab ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'
                    }`}
                >
                    {tab}
                </button>
      ...
      {/* Content */}
      <div className="flex-grow overflow-auto p-6 custom-scrollbar bg-[#0f172a]">
